...
function upsert(dic) {
  ...
  dic.events.forEach(function(ev) {
    ...
    var notionRes = null;
    if (token && dbId) {
      notionRes = _gl_upsertNotionPage_(ev, token, dbId, normalizedColor, reminderTexts, notionSchema, notionLookupCache);
...
function _gl_upsertNotionPage_(ev, token, dbId, colorValue, reminderTexts, notionSchema, notionLookupCache) {
  ...
  var resp = UrlFetchApp.fetch("https://api.notion.com/v1" + path, {
    method: method,
    headers: {
...
function _gl_getNotionDbSchema_(token, dbId) {
  ...
  var resp = UrlFetchApp.fetch("https://api.notion.com/v1/databases/" + dbId, {
    headers: {
      "Authorization": "Bearer " + token,
      "Notion-Version": "2022-06-28"
    },
